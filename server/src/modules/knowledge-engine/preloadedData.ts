...
function generatePreloadedParameters(): PreloadedParameter[] {
  ...
  // 4. Generate Allergen IgE Specifics (600 parameters)
  for (let i = 1; i <= 600; i++) {
    const allergenName = `Allergen Spec IgE ${i}`;
    const code = `IGE-E${i}`;
    params.push({
      name: allergenName,
      shortCode: code,
      aliases: [`Allergen E${i}`, `IgE ${i}`],
      category: 'Immunology',
      unitName: 'kU/L',
      decimalPrecision: 2,
      description: `Specific IgE antibody level for ${allergenName}`,
      referenceRanges: [
        { gender: 'ALL' as const, ageMin: 0, ageMax: 120, minVal: 0.0, maxVal: 0.35, displayText: '< 0.35 kU/L', condition: 'ADULT' }
      ]
    });
  }

  // 5. Generate Antibiotic Sensitivity Parameters (1000 parameters)
  // 50 antibiotics * 20 specimen types
  const antibiotics = [
    'Amikacin', 'Amoxicillin', 'Ampicillin', 'Azithromycin', 'Aztreonam', 'Cefazolin', 'Cefepime', 'Cefotaxime', 
    'Cefoxitin', 'Ceftazidime', 'Ceftriaxone', 'Cefuroxime', 'Chloramphenicol', 'Ciprofloxacin', 'Clindamycin', 
    'Colistin', 'Erythromycin', 'Fosfomycin', 'Gentamicin', 'Imipenem', 'Linezolid', 'Meropenem', 'Minocycline', 
    'Moxifloxacin', 'Nitrofurantoin', 'Norfloxacin', 'Ofloxacin', 'Oxacillin', 'Penicillin G', 'Piperacillin', 
    'Piperacillin-Tazobactam', 'Polymyxin B', 'Rifampin', 'Tetracycline', 'Tobramycin', 'Trimethoprim-Sulfamethoxazole', 
    'Vancomycin', 'Clarithromycin', 'Levofloxacin', 'Teicoplanin', 'Tigecycline', 'Doxycycline', 'Netilmicin', 
    'Sulbactam', 'Fusidic Acid', 'Mupirocin', 'Cefadroxil', 'Cefixime', 'Cefpodoxime', 'Ertapenem'
  ];

  const specimens = [
    { name: 'Urine', code: 'URN' }, { name: 'Blood', code: 'BLD' }, { name: 'Pus', code: 'PUS' }, 
    { name: 'Sputum', code: 'SPT' }, { name: 'CSF', code: 'CSF' }, { name: 'Wound', code: 'WND' },
    { name: 'Throat', code: 'THR' }, { name: 'Stool', code: 'STL' }, { name: 'Ear', code: 'EAR' },
    { name: 'Eye', code: 'EYE' }, { name: 'Nasal', code: 'NSL' }, { name: 'Vaginal', code: 'VAG' },
    { name: 'Urethral', code: 'URT' }, { name: 'Semen', code: 'SMN' }, { name: 'Synovial', code: 'SYN' },
    { name: 'Pleural', code: 'PLR' }, { name: 'Ascitic', code: 'ASC' }, { name: 'Pericardial', code: 'PRC' },
    { name: 'Dialysate', code: 'DLS' }, { name: 'Tissue', code: 'TIS' }
  ];

  for (const ab of antibiotics) {
    for (const sp of specimens) {
      const name = `${ab} Sensitivity (${sp.name})`;
      // Use clean string helper to generate unique shortcode
      const cleanAbName = ab.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
      const code = `SEN-${cleanAbName.substring(0, 6)}-${sp.code}`;
      params.push({
        name,
        shortCode: code,
        aliases: [`${ab} ${sp.name}`, name],
        category: 'Microbiology',
        unitName: 'Neg/Pos',
        decimalPrecision: 0,
        description: `Antibiotic susceptibility channel for ${ab} on ${sp.name} isolate`,
        referenceRanges: [
          { gender: 'ALL' as const, ageMin: 0, ageMax: 120, displayText: 'Sensitive / Intermediate / Resistant', condition: 'ADULT' }
        ]
      });
    }
  }

  // 6. Generate Microbiology Organisms (1000 parameters)
  // 50 organisms * 20 specimen types
  const organisms = [
    'Escherichia coli', 'Klebsiella pneumoniae', 'Pseudomonas aeruginosa', 'Staphylococcus aureus', 
    'Enterococcus faecalis', 'Enterococcus faecium', 'Streptococcus pneumoniae', 'Streptococcus pyogenes', 
    'Streptococcus agalactiae', 'Neisseria gonorrhoeae', 'Neisseria meningitidis', 'Haemophilus influenzae', 
    'Proteus mirabilis', 'Proteus vulgaris', 'Enterobacter cloacae', 'Serratia marcescens', 
    'Acinetobacter baumannii', 'Stenotrophomonas maltophilia', 'Salmonella typhi', 'Salmonella enteritidis', 
    'Shigella sonnei', 'Shigella flexneri', 'Vibrio cholerae', 'Campylobacter jejuni', 'Helicobacter pylori', 
    'Clostridioides difficile', 'Bacteroides fragilis', 'Candida albicans', 'Candida tropicalis', 
    'Candida glabrata', 'Candida krusei', 'Aspergillus fumigatus', 'Aspergillus flavus', 
    'Cryptococcus neoformans', 'Mycobacterium tuberculosis', 'Mycobacterium avium', 'Nocardia asteroides',
    'Morganella morganii', 'Providencia stuartii', 'Citrobacter freundii', 'Citrobacter koseri',
    'Hafnia alvei', 'Serratia liquefaciens', 'Yersinia enterocolitica', 'Vibrio vulnificus',
    'Vibrio parahaemolyticus', 'Aeromonas hydrophila', 'Plesiomonas shigelloides', 'Moraxella catarrhalis',
    'Bordetella pertussis'
  ];

  for (const org of organisms) {
    for (const sp of specimens) {
      const name = `${org} Isolate (${sp.name})`;
      const cleanOrgName = org.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
      const code = `ORG-${cleanOrgName.substring(0, 8)}-${sp.code}`;
      params.push({
        name,
        shortCode: code,
        aliases: [`${org} ${sp.name}`, name],
        category: 'Microbiology',
        unitName: 'Neg/Pos',
        decimalPrecision: 0,
        description: `Microbiology culture screening for ${org} from ${sp.name}`,
        referenceRanges: [
          { gender: 'ALL' as const, ageMin: 0, ageMax: 120, displayText: 'Not Detected / Growth Observed', condition: 'ADULT' }
        ]
      });
    }
  }
